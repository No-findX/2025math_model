...
try:
    ...
    print(f"错误: 未找到数据文件 '{path}'。请确保文件在正确的目录下。")
    exit()
...
# ========== 2. 优化后的质量控制（QC Gate） ==========
print("\n[步骤 2/8] 正在执行优化后的质量控制 (GC阈值=0.395)...")
...
feature_cols = [
    # 1. 核心预测变量 (经数据验证，P<0.05)
    "X染色体浓度_标准",
    "21号染色体的GC含量_标准",
    "13号染色体的GC含量_标准",

    # 2. 关键控制变量 (基于领域知识和理论重要性)
    "21号染色体的Z值_标准",
    "18号染色体的Z值_标准",
    "孕妇BMI",
    "孕周数值"
]
print(f"模型将使用以下 {len(feature_cols)} 个特征 (核心预测+关键控制):")
...
print(f"计算出的最优决策点 t* = {t_star:.3f}")
print(f"最终双阈值设置: t_low={t_low:.3f}, t_high={t_high:.3f}")

# ========== 6. 最终训练与三分类判定 ==========
print("\n[步骤 6/8] 正在使用平衡模型进行最终训练并生成预测...")
...
print(f"\n--- 流程执行完毕 ---")
print(f"平衡策略模型的最终结果已保存至文件: {output_filename}")